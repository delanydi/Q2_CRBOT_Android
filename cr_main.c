...
CRBOT_INLINE float DotProduct( vec3_t x, vec3_t y) { return (x[0]*y[0]+x[1]*y[1]+x[2]*y[2]); }
CRBOT_INLINE void VectorSubtract( vec3_t a, vec3_t b, vec3_t c) { c[0]=a[0]-b[0]; c[1]=a[1]-b[1]; c[2]=a[2]-b[2]; }
CRBOT_INLINE void VectorAdd( vec3_t a, vec3_t b, vec3_t c) { c[0]=a[0]+b[0]; c[1]=a[1]+b[1]; c[2]=a[2]+b[2]; }
CRBOT_INLINE void VectorCopy( vec3_t a, vec3_t b) { b[0]=a[0]; b[1]=a[1]; b[2]=a[2]; }
CRBOT_INLINE void VectorClear( vec3_t a) { a[0]=a[1]=a[2]=0; }
CRBOT_INLINE void VectorNegate(vec3_t a, vec3_t b) { b[0]=-a[0]; b[1]=-a[1]; b[2]=-a[2]; }

CRBOT_INLINE qboolean bVectorZero( vec3_t a ) { return fabs(a[0])<0.05f && fabs(a[1])<0.05f && fabs(a[2])<0.05f; }
...
CRBOT_INLINE float dist( vec3_t x, vec3_t y )  { return sqrt(sqr(x[0]-y[0])+sqr(x[1]-y[1])+sqr(x[2]-y[2])); }
CRBOT_INLINE float dist2( vec3_t x, vec3_t y ) { return sqr(x[0]-y[0])+sqr(x[1]-y[1])+sqr(x[2]-y[2]); }

CRBOT_INLINE float dist2d( vec3_t x, vec3_t y )  { return sqrt(sqr(x[0]-y[0])+sqr(x[1]-y[1])); }
CRBOT_INLINE float dist2d2( vec3_t x, vec3_t y ) { return sqr(x[0]-y[0])+sqr(x[1]-y[1]); }
